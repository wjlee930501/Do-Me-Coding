...
- The "CI green on branch" acceptance criterion is verifiable only after the human push gate (the workflow is inert until pushed); disclosed and accepted in the plan §Approval Status sequencing note. Locally, the workflow parses cleanly, fetch-depth 0 is pinned, and both CI greps (AA1 forbidden-lexeme, CF3 model-name) return empty over their scoped paths today.
  - **RESOLVED 2026-07-08 at `4114a6b` (Actions run `28899008386` = success).** All 13 blocking checks green (mirror-check, doctor, `selftest release-gate` 39/0, `selftest m9-suite` 56/0+35/0, linkcheck, CF3 + AA1 greps, Codex-wiring presence, porcelain PRE/MID). The full legacy `selftest --all` replay is ADVISORY (continue-on-error): six workflow-file-only fix-forwards proved the pinned 802/3/3 baseline is a macOS-dev-environment artifact that no GitHub runner reproduces EXACTLY (ubuntu 799/6, macos-latest 800/5) — the divergence is 2–3 frozen mirror-pinned legacy tools (v0.2.6/v0.3.9/v0.3.1) whose `git status --porcelain` cleanliness assertions react to runner OS/py-patch bytecode behavior, which M9 cannot edit. Root cause: verify tools call `python3 -m py_compile`, which ignores `PYTHONDONTWRITEBYTECODE` but honors `PYTHONPYCACHEPREFIX` (now set to `/tmp/dmc-pycache`); that plus the python-3.9 pin lifted CI from 796/9 to within the pinned 3. The remaining 2–3 are OS-portability, deferred as an **M10 carry-forward**. The definitive 802/3/3 proof stays the maintainer's local + committed-replica run recorded above.
