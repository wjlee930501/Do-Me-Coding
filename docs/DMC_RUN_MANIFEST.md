...
| `protected_paths[]` | default (env `DMC_GATE_PROTECTED`) | adapters/router/schemas/hooks/`dmc-glm-smoke` |
...
- **Read-only / no mutation.** The generator issues only read git commands and reads the named plan; `--out` writes one
  manifest file to the operator-named path and does not `git add` it. The only writes anywhere are inside `--self-test`'s
  temp repos.
