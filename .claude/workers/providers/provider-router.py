...
- Static REGISTRY table keyed by (type, provider). Exact match; empty provider matches only if the type has exactly
  one adapter; unknown / missing / mock / empty -> refuse (deterministic, no guess). (v0.3.2: manual_import routes to the
  standalone manual-import adapter — no live mode; input via --import.)
...
REGISTRY = {
    ...
    # v0.3.2: manual_import is a standalone pure-validation importer — NO live mode (live_flag=None); input via --import.
    ("manual_import", "manual-import"): {"adapter": "manual-import/manual-import-adapter.py", "live_flag": None},
}
...
def select_entry(task):
    ...
    if ptype in ("", "mock"):
        die(f"provider_target.type={ptype!r} has no adapter to route to (refusing)")
    if provider:
...
def main():
    ap = argparse.ArgumentParser(allow_abbrev=False)   # exact dispatch-flag surface (no prefix abbreviation)
    ap.add_argument("--task", required=True)
    ap.add_argument("--mock")
    ap.add_argument("--import", dest="import_")        # v0.3.2: manual_import input path (router enforces no required; adapter does)
    ...
    # v0.3.2 cross-flag rejection (router owns its contract; refuse BEFORE exec, not via the adapter-argparse backstop):
    # manual_import uses --import (no --mock); the network/exec providers use --mock (no --import). Legit combos allowed.
    is_manual = entry["live_flag"] is None
    if a.mock and is_manual:
        die(f"--mock is not valid for (type={ptype}, provider={provider}); manual_import uses --import (refusing)")
    if a.import_ and not is_manual:
        die(f"--import is not valid for (type={ptype}, provider={provider}); use --mock (refusing)")
    ...
    if a.mock:
        argv += ["--mock", a.mock]
    if a.import_:
        argv += ["--import", a.import_]
    if a.out:
        argv += ["--out", a.out]
    if a.live:
        if entry["live_flag"] is None:   # v0.3.2 no-live guard (BEFORE reading required): manual_import has no live mode
            die(f"--live not supported for (type={ptype}, provider={provider}) — refusing")
