...
---
<!-- DMC-HOST-NOTE:BEGIN -->
Host: Codex CLI. This SKILL.md lives at `.agents/skills/dmc-start-work/SKILL.md`. Frontmatter
is Codex-standard (`name`, `description` only). Invocation is EXPLICIT — run
`$dmc-start-work <path to approved plan>` or pick `dmc-start-work` from `/skills`; Codex never
auto-dispatches a skill (mirrors the Claude Code skill's `disable-model-invocation: true`). The
approved plan path takes the place of `$ARGUMENTS` below.
<!-- DMC-HOST-NOTE:END -->
