...
---
<!-- DMC-HOST-NOTE:BEGIN -->
Host: Codex CLI. This SKILL.md lives at `.agents/skills/dmc-critic/SKILL.md`. Frontmatter is
Codex-standard (`name`, `description` only). Invocation is EXPLICIT — run
`$dmc-critic <path to plan>` or pick `dmc-critic` from `/skills`; Codex never auto-dispatches a
skill (mirrors the Claude Code skill's `disable-model-invocation: true`). The plan path takes
the place of `$ARGUMENTS` below. Codex's skill frontmatter has no `disallowed-tools` field
(unlike the Claude Code counterpart's `disallowed-tools: Edit, Write`); the read-only/advisory
constraint below is instead carried entirely by the operative instructions themselves and by
the Ring-0 role registry (`orchestration/roles.json`: `critic-falsifier` is not `may_mutate`).
<!-- DMC-HOST-NOTE:END -->
