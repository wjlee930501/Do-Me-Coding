...
---
<!-- DMC-HOST-NOTE:BEGIN -->
Host: Codex CLI. This SKILL.md lives at `.agents/skills/dmc-status/SKILL.md`. Frontmatter is
Codex-standard (`name`, `description` only). Invocation is EXPLICIT — run `$dmc-status` or pick
`dmc-status` from `/skills`; Codex never auto-dispatches a skill (mirrors the Claude Code
skill's `disable-model-invocation: true`). This skill takes no arguments.
<!-- DMC-HOST-NOTE:END -->
