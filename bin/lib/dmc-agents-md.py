...
def derive_facts(root):
    ...
    # Per-class counts over the SAME landmark list section 4 renders (enforcement / contract /
    # release). The compact section-5 render reports these counts instead of re-inlining every
    # path; because they are counted from the §4 landmark list, the count-parity selftest can
    # re-derive them straight from the rendered §4 and confirm the compacted §5 counts match.
    protected_landmark_counts = {"enforcement": 0, "contract": 0, "release": 0}
    for m in landmarks:
        if isinstance(m, dict) and m.get("path"):
            cls = m.get("class")
            if cls in protected_landmark_counts:
                protected_landmark_counts[cls] += 1
    ...
    return {
        ...
        "protected_landmark_counts": protected_landmark_counts,
        "risk": risk,
...
def render_sections(facts, unknowns):
    ...
    Emission is INVENTORY-LAST — physical order [1,2,3,6,7,8,9,10,4,5]. Every section keeps its
    pinned numeric label and title; only the big architecture-landmark inventory (§4) and the
    protected-surfaces list (§5) relocate to the tail, after §10. This puts the behavioral rules
    (§7 operating rules, §9 stop conditions) and all operational sections physically BEFORE the
    inventory, so a host that truncates past its byte cap drops the inventory tail, not the rules.
    The validator is order-independent (split_sections keys by number), so the doc stays VALID."""
    header = [
    ...
    sec1 = _section(1, "Repo identity", [
        "- Name: %s" % facts["name"],
        "- Purpose: %s" % facts["purpose"],
    ])

    sec2 = _section(2, "Stack and package manager", [
    ...
    sec3 = _section(3, "Lint / typecheck / test / build commands", cmd_lines)
    ...
    if facts["landmarks"]:
        ...
        lm_lines = ["- %s" % UNKNOWN]
    sec4 = _section(4, "Architecture landmarks", lm_lines)
    ...
    if facts["protected_landmarks"]:
        # Compact cross-reference, not a second full enumeration: §4 already lists every path, so
        # §5 carries only the "(see section 4)" pointer plus a per-class count. The count-parity
        # selftest re-derives these counts from the rendered §4 and confirms they match.
        counts = facts["protected_landmark_counts"]
        prot_lines.append(
            "- Repository enforcement / contract / release landmarks (see section 4): "
            "%d enforcement / %d contract / %d release landmarks."
            % (counts["enforcement"], counts["contract"], counts["release"]))
    sec5 = _section(5, "Protected surfaces", prot_lines)

    sec6 = _section(6, "Migration / env / auth / billing risk notes", [
    ...
    sec7 = _section(7, "DMC operating rules", section7_body)

    sec8 = _section(8, "Verification commands", [
    ...
    sec9 = _section(9, "Stop conditions", [
    ...
    sec10 = _section(10, "Explicit Unknowns", unk_lines)

    # Inventory-last emission [1,2,3,6,7,8,9,10,4,5]: rules and operational sections first, the
    # large §4/§5 inventory last (see the module docstring above for the rationale).
    out = header + sec1 + sec2 + sec3 + sec6 + sec7 + sec8 + sec9 + sec10 + sec4 + sec5
...
def _section_span(text, num):
    """Return (start, end) char offsets of the `## num.` section block — from its own heading to
    the START of the next EMITTED `## K.` heading (whatever K is), or EOF when it is emitted last.
    Located purely by emitted order via HEADING_RE, never a hardcoded successor number; None if the
    section is absent. Lets order-dependent negative controls slice a section out (or blank its
    body) without assuming any physical adjacency."""
    starts = []
    pos = 0
    for line in text.splitlines(keepends=True):
        m = HEADING_RE.match(line.rstrip("\n"))
        if m:
            starts.append((pos, int(m.group(1))))
        pos += len(line)
    for i, (off, n) in enumerate(starts):
        if n == num:
            return (off, starts[i + 1][0] if i + 1 < len(starts) else len(text))
    return None


def _s4_protected_counts(text):
    """Re-derive §4's enforcement / contract / release landmark counts straight from the RENDERED
    §4 list (not from the facts object) — the count-parity guard compares this to §5's compact
    line, so a wrong or off-by-one compacted §5 count cannot pass."""
    body = split_sections(text).get(4, (None, ""))[1]
    counts = {"enforcement": 0, "contract": 0, "release": 0}
...
def _s5_protected_counts(text):
    """Parse §5's compacted per-class protected-landmark counts back into a dict, or None if the
    section carries no protected-landmark line (an empty repo)."""
    body = split_sections(text).get(5, (None, ""))[1]
    m = re.search(r"(\d+) enforcement / (\d+) contract / (\d+) release", body)
    if not m:
        return None
    return {"enforcement": int(m.group(1)), "contract": int(m.group(2)),
...
def selftest():
    ...
    try:
        ...
        s10_span = _section_span(empty_doc, 10)
        t.ok("E4 empty fixture Unknowns list is non-empty and names package manager",
             s10_span is not None
             and "package manager" in empty_doc[s10_span[0]:s10_span[1]])
        ...
        # --- count-parity guard (A1): §5's compacted per-class counts must equal a count re-derived
        #     straight from the rendered §4 landmark list (enforcement / contract / release). Both
        #     sides are PARSED from the rendered document — never both from `facts` — so a wrong or
        #     off-by-one compacted §5 count cannot pass. The fixture carries exactly one landmark of
        #     each protected class (.github/workflows => enforcement, package.json => contract,
        #     docs/MILESTONES.md => release). ---
        parity = os.path.join(tmp, "parity")
        os.makedirs(os.path.join(parity, ".github", "workflows"))
        os.makedirs(os.path.join(parity, "docs"))
        _write(os.path.join(parity, ".github", "workflows", "ci.yml"), "name: ci\n")
        _write(os.path.join(parity, "package.json"), "{}\n")
        _write(os.path.join(parity, "docs", "MILESTONES.md"), "# milestones\n")
        parity_doc, _ = generate(parity)
        s5_counts = _s5_protected_counts(parity_doc)
        s4_counts = _s4_protected_counts(parity_doc)
        t.ok("PC1 §5 compacted per-class counts equal the §4-derived counts (count-parity guard)",
             s5_counts is not None and s5_counts == s4_counts
             and s4_counts == {"enforcement": 1, "contract": 1, "release": 1})

        # --- validator negative controls ---
        # (a) missing section — locate §6 by its own heading and the next EMITTED heading (via
        #     _section_span), never a hardcoded successor number, so the control survives the
        #     inventory-last reorder and any future one.
        span6 = _section_span(node_doc, 6)
        cut = node_doc[:span6[0]] + node_doc[span6[1]:] if span6 else node_doc
        cut_ok, cut_msgs = validate_doc(cut)
        t.ok("V1 validator REFUSES a doc with section 6 deleted (missing section)",
             span6 is not None and (not cut_ok)
             and any("missing required section 6" in m for m in cut_msgs))
        ...
        # (d) empty section body — blank §9's body in place, locating the block by emitted order
        #     (_section_span) so the control does not assume §10 physically follows §9. The heading
        #     is retained and its body removed, which the validator flags as an empty section.
        span9 = _section_span(node_doc, 9)
        heading9 = node_doc[span9[0]:span9[1]].splitlines()[0] if span9 else ""
        emptied = (node_doc[:span9[0]] + heading9 + "\n\n" + node_doc[span9[1]:]
                   if span9 else node_doc)
        emptied_ok, emptied_msgs = validate_doc(emptied)
        t.ok("V4 validator REFUSES an empty required section body",
             span9 is not None and (not emptied_ok)
             and any("is empty" in m for m in emptied_msgs))
